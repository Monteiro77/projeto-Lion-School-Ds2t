...
app.get('/v1/lion-school/alunos', cors(), async function (request, response, next) {
    ...
    if (siglaCurso != undefined && statusAluno != undefined) {
        ...
        if (siglaCurso == "" || siglaCurso == undefined || !isNaN(siglaCurso) && siglaCurso == "" || siglaCurso == undefined || !isNaN(siglaCurso)) {
            statusCode = 400
            dadosAluno.message = 'Não foi possível fazer a busca, pois os dados de entrada(curso) estão incorretos'
        } else {
            aluno = cursos.getAlunoCursoStatus(siglaCurso, statusAluno)
        }
